...
void ubx_prot_arraymake(double *x, double *y, uint8_t *sign, double *theta, double *delta_x, double *delta_y, double *delta_theta)
{
	...
	*theta = *theta + *delta_theta;
	...
	uint32_t temp_x = (uint32_t)(*x*100000);
	uint32_t temp_y = (uint32_t)(*y*100000);
	uint32_t temp_theta = (uint32_t)(*theta*100000);
	
// 	char write_string[50];
// 	sprintf(write_string,"x=%4d, y=%4d, theta=%4d", (uint16_t)x, (uint16_t)y, (uint16_t)theta);
	//uart_0_write_string(write_string);
